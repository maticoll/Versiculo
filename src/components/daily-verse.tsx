...
async function fetchDailyVerseData(): Promise<DailyVerseData> {
  try {
    ...
    return { text, reference, bookName: book?.name };
  } catch {
    return null;
  }
}
...
export async function DailyVerse() {
  const data = await fetchDailyVerseData();
  if (!data) return null;
    ...
    <section className="space-y-3">
      ...
      <Card className="border-[var(--accent-bible)]/30 bg-[var(--accent-bible)]/5">
        <CardHeader className="pb-2">
          <p className="text-sm font-medium text-[var(--accent-bible)]">{data.reference}</p>
        </CardHeader>
        <CardContent>
          <p className="font-serif text-xl leading-relaxed md:text-2xl">{data.text}</p>
          <p className="mt-2 text-xs text-muted-foreground">{data.bookName} — {DEFAULT_TRANSLATION}</p>
        </CardContent>
      </Card>
    </section>
  );
